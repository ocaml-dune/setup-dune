...
`traces-as-artifact` can take the values `never`, `on-failure` (which is its
default) or `always` to control when the traces generated by Dune should be made
available as an artifact to investigate off-line.
